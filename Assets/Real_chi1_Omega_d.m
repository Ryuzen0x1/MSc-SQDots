...
plot(deltap./10.^11, real(chi1),'-.', LineWidth=2.5, DisplayName='\Omega_d=2.0', Color=[0 0 1]); % Blue
hold on;
...
%Y=abs(omega_d)*abs(omega_c);
%chi3 =- l*chi1*((abs(Dp).^2 + X + Y.^2)/ abs(D).^2);
plot(deltap./10.^11, real(chi1), LineWidth=2.5, DisplayName='\Omega_d=4.0', Color=[1 0 0]); % Red
hold on;
...
%Y=abs(omega_d)*abs(omega_c);
%chi3 =- l*chi1*((abs(Dp).^2 + X + Y.^2)/ abs(D).^2);
plot(deltap./10.^11, real(chi1), LineWidth=2.5, DisplayName='\Omega_d=6.0'); % Yellow
hold on;
...
%Y=abs(omega_d)*abs(omega_c);
%chi3 =- l*chi1*((abs(Dp).^2 + X+ Y.^2)/ abs(D).^2);
plot(deltap./10.^11, real(chi1), '--', LineWidth=2.5, DisplayName='\Omega_d=8.0'); % Purple
hold on;
...
Y=abs(omega_d)*abs(omega_c);
chi3 =- l*chi1*((abs(Dp).^2 + X + Y.^2)/ abs(D).^2);
plot(deltap./10.^11, real(chi1), LineWidth=2.5, DisplayName='\Omega_d=10.0', Color=[0 0.5 0]); % Green
hold on;

lgd = legend('show');
set(lgd, 'FontName', 'Garamond', 'FontSize', 24, 'FontWeight', 'bold', 'EdgeColor', ...
    'w', 'NumColumns', 2, TextColor=[0 0 0]);
legend boxon;
...
%title('\Delta_p vs Re\chi^{(1)}', FontName='Garamond', FontSize=27, FontWeight='bold');
grid on;

folder_path = "C:\Users\Ryuki\Downloads\PLOTS";
file = fullfile(folder_path, 'Real_chi1_Omega_d.jpeg');
fig = gcf;
